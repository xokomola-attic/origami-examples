...
{
    concat('file://',file:base-dir(), 'json/', $name)
};

declare function test:read($name)
{
    μ:parse-json(μ:read-json(test:json($name)))
};

declare function test:read($name, $options)
{
    μ:parse-json(μ:read-json(test:json($name), $options))
};

declare function test:parse($name)
{
    μ:parse-json(unparsed-text(test:json($name)))
};

declare %unit:test function test:read-csv() 
{
    let $json := test:read('countries.json')
    return (
        unit:assert-equals(μ:size($json), 4)
    )
};

declare %unit:test function test:parse-csv() 
{
    let $json := test:parse('countries.json')
    return (
        unit:assert-equals(μ:size($json), 4)
    )
};
